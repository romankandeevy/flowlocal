...
_PARASITES ниже. Дальше самоисправления (corrections.py): «в пятницу, нет, в
субботу» -> «в субботу», тоже правилами и тоже по запятым. Опциональный слой -
локальная LLM через Ollama (config.llm.enabled): за ней остался тон и те
поправки, которые правилам не даются. Коробочную LLM пробовали и отвергли
замером: PLAN 2.6-бис.
Последний слой - сниппеты (config.snippets): подстановка "сказать -> вставить"
для адресов и реквизитов, которые надоело диктовать по буквам. В том, на что
заменяем, работают четыре переменные - {дата}, {дата-словами}, {время},
{день-недели}, - иначе дату в подписи приходилось править руками после каждой
вставки.
...
# задача, тем меньше простора напортить. Простые поправки до неё разбирает
# corrections.py правилами; LLM остаётся то, где нужна параллель без явных
# признаков, и она уже не обязательна. Слова неуверенности защищаем прямо в
...
def autoconfig_llm(cfg: dict, log) -> str:
    ...
    if not have:
        if llm.get("enabled"):
            # В журнал, и только в журнал. Диктовка от этого не страдает -
            # знаки и оговорки давно чинят правила, - поэтому прерывать
            # человека сообщением посреди работы незачем. Сказать всё равно
            # надо, но там, где он придёт спрашивать: окна показывают
            # «Правка текста не отвечает» с кнопкой (ollama_setup.state()).
...
# Разметка жирного и курсива: **так**, ***так***, *так*, __так__.
#
# Одиночное подчёркивание НЕ трогаем намеренно (потому `_{2,3}`, а не `_{1,3}`):
# оно живёт внутри имён - `foo_bar`, `send_mail`, - и пара таких имён в одной
# фразе выглядела бы для выражения как курсив, а вычеркнуть подчёркивания в
# именах хуже, чем оставить редкий курсив от модели.
#
# Требуем непробельный символ сразу после открывающей пары и перед закрывающей -
# иначе выражение съело бы «5 * 3 = 15», приняв звёздочки за разметку.
_MD_EMPHASIS = re.compile(r"(\*{1,3}|_{2,3})(?=\S)(.+?)(?<=\S)\1", re.DOTALL)


def _strip_md(before: str, after: str) -> str:
    """Снять разметку, которой в исходнике не было.

    Владелец пожаловался: в надиктованном тексте «рандомно, без всякой
    закономерности» появляются пары звёздочек вокруг слов. Закономерности и
    правда нет - это вкусовщина LLM: её просят причесать фразу, а она заодно
    выделяет жирным то, что сочла важным. В его записи так вышло «Править
    выделенное текстом **это** непонятно... что это **как** используется».

    Почему это проскочило мимо обоих предохранителей. `_is_deletion_of`
    сравнивает НОРМАЛИЗОВАННЫЕ слова, и `**это**` превращается в `это` -
    проверка довольна. У тона такой проверки нет вовсе: ему переписывать
    разрешено. То есть дыра была в обоих проходах сразу.

    Почему снимать безопасно: GigaAM звёздочек и подчёркиваний не выдаёт
    никогда - в её словаре их нет. Значит любая разметка в ответе появилась
    после модели. Но полагаться на это вслепую всё же не будем: если в
    исходнике знак ВСТРЕЧАЛСЯ (подстановка с формулой, путь `foo_bar`), не
    трогаем ничего. Лучше оставить лишнюю звёздочку, чем испортить то, что
    человек продиктовал сам.

    Промптом это не лечится: попросить «не используй разметку» можно, но
    послушность - надежда, а не гарантия. Здесь же вычёркивание, и оно не может
    потерять слова: снимаются только сами знаки, текст между ними остаётся.
    """
    if not after or "*" in before or "_" in before:
        return after
    out, prev = after, None
    while out != prev:      # вложенное «***важно***» снимается не за один проход
        prev = out
...
def _llm_polish(text: str, llm_cfg: dict, log) -> str:
    ...
    # Разметку снимаем ДО проверок: иначе `_is_deletion_of` сверяет слова со
    # звёздочками, а `_sane` считает их частью текста.
    got = _strip_md(text, got)
...
def _llm_tone(text: str, tone: str, llm_cfg: dict, log) -> str:
    ...
    # Тону позволено больше: он для того и зван, чтобы переписать фразу. Но
    # переписать - не значит разметить: жирное и курсив снимаем и здесь, причём
    # именно тон разметку и любит, потому что его просят «сделать строже».
    got = _strip_md(text, got)
...
# --- Переменные в подстановках --------------------------------------------
#
# Ровно четыре, и новых не заводим. Каждая отвечает на вопрос «что мне писать
# руками после вставки», и других таких вопросов у подписи в письме нет.
# Список короткий намеренно: плейсхолдер, который надо смотреть в справке, -
# это уже язык программирования, а подстановки заводят люди, которым надоело
# диктовать свой адрес по буквам.
#
# Месяцы и дни недели своей таблицей, а не через strftime("%B"): формат
# зависит от локали системы, и на английской Windows «19 июля» превратилось бы
# в «19 July». Нам нужен ещё и родительный падеж, которого в локали нет вовсе.
...
_VARS = {
    "дата": lambda t: time.strftime("%d.%m.%Y", t),
    "дата-словами": lambda t: f"{t.tm_mday} {_MONTHS_GEN[t.tm_mon - 1]}",
    "время": lambda t: time.strftime("%H:%M", t),
    "день-недели": lambda t: _WEEKDAYS[t.tm_wday],
}

# Имя внутри скобок ограничено по длине: без ограничения открывающая скобка в
# начале строки нашла бы закрывающую через полтекста и съела бы всё между ними.
_VAR_RE = re.compile(r"\{([^{}]{1,20})\}")


def _fill_vars(value: str, now=None) -> str:
    """Подставить {дата}, {время} и прочее В ЗНАЧЕНИИ подстановки.

    Только в значении, и это главное свойство. Раскрывать переменные во всём
    тексте нельзя: тогда продиктованное «поставь скобку дата скобку» (или уже
    подставленный кусок чужого конфига) внезапно превращалось бы в дату. Мы
    трогаем ровно то, что человек сам написал в поле «на что заменить».

    Неизвестное имя остаётся как есть: «{чушь}» - это либо опечатка, либо
    фигурные скобки в настоящем тексте, и в обоих случаях выбрасывать кусок
    чужой строки мы не имеем права.
    """
    if "{" not in value:
        return value
    ...
    def one(m):
        ...
        try:
            return fn(t)
        except Exception:  # noqa: BLE001 - битое время не стоит подстановки
            return m.group(0)

    return _VAR_RE.sub(one, value)


def _apply_snippets(text: str, snippets: dict, now=None) -> str:
    ...
    now берётся один раз на всю фразу, а не на каждую переменную: подстановка
    с {дата} и {время} рядом обязана показать один момент времени, а не два
    (иначе раз в сутки на границе полуночи выйдет вчерашняя дата с сегодняшним
    временем). Заодно этим параметром тест закрепляет ожидаемый вид.
    """
    ...
    t = now or time.localtime()
    return pattern.sub(lambda m: _fill_vars(by_lower[m.group(0).lower()], t), text)
...
#
# Запятая ВНУТРИ, после предлога «с»: знаки ставит пунктуатор, и ставит их ДО
# того, как ищется команда (extract_commands зовётся после clean). Одиночный
# предлог он охотно отделяет запятой - «с, новой строки», - и без терпимости к
# ней команда не опознавалась и писалась словами. Поймано на живой диктовке:
# владелец надиктовал перенос, получил текст с запятыми внутри.
_CMD_NEWLINE = re.compile(
    r"[ \t]*,?[ \t]*\b(?:с[ \t]*,?[ \t]*новой\s+строки|перенос\s+строки)\b"
    r"[ \t]*[,.;:]?[ \t]*",
    re.IGNORECASE)
_CMD_PARAGRAPH = re.compile(
    r"[ \t]*,?[ \t]*\bс[ \t]*,?[ \t]*нового\s+абзаца\b[ \t]*[,.;:]?[ \t]*",
    re.IGNORECASE)
...
# Дата и время голосом - то же самое, что переменные в подстановках, только
# заводить подстановку не надо: это нужно всем и одинаково.
#
# Формы ровно две и обе именительные, по тому же правилу, что у «с новой
# строки»: «сегодняшней датой» и «текущего времени» - это уже речь о дате, а не
# просьба её вставить, и подменять там слова значило бы портить текст. Знаки
# вокруг не съедаем: команда стоит внутри фразы («встреча сегодняшняя дата в
# десять»), а не вместо неё.
_CMD_DATE = re.compile(r"\bсегодняшняя\s+дата\b", re.IGNORECASE)
_CMD_TIME = re.compile(r"\bтекущее\s+время\b", re.IGNORECASE)


def extract_commands(text: str, cfg: dict, now=None) -> tuple[str, bool]:
    ...
    now - тот же приём, что в _apply_snippets: одно время на всю фразу, и тест
    может его закрепить.
    """
    try:
        if not text or not cfg.get("voice_commands", True):
            return text, False
        t = now or time.localtime()
        text = _CMD_DATE.sub(lambda _m: _VARS["дата"](t), text)
        text = _CMD_TIME.sub(lambda _m: _VARS["время"](t), text)
        press_enter = False
...
def _punctuate(text: str, cfg: dict) -> str:
    """Расставить знаки - но только если модель распознавания их не ставит.

    Гейт по модели, а не по виду текста, и это принципиально. Вариант e2e
    расставляет знаки сам и местами осознанно НЕ ставит запятую там, где
    правило её поставит. Наши правила поверх дали бы дубли и спор с моделью.

    Проверка по тексту («есть ли уже запятые») остаётся ниже вторым рубежом,
    но одна она недостаточна: короткая фраза от e2e может выйти вовсе без
    знаков, и правила решили бы, что перед ними сырой текст.

    Уровень 1 (punct_rules): правила, доли миллисекунды, ноль зависимостей.
    Уровень 2 (модель) заменит его, когда будет установлена; правила останутся
    под ней запасным вариантом.

    Ollama из этого пути убрана намеренно: она ставила знаки за 0.34 с - дольше,
    чем всё распознавание. Самоисправления у неё тоже забрали правила
    (corrections.py), так что обязательной работы у неё не осталось - только тон.
    """
    try:
        import models
...
def clean(text: str, cfg: dict, log, process: str = "") -> str:
    ...
    try:
        ...
        # Знаки препинания - ДО фильтра бубнежа, и это не мелочь порядка.
        ...
        # Самоисправления - после знаков и после фильтра бубнежа. После знаков
        # потому, что маркер поправки опознаётся по запятым вокруг него, а до
        # пунктуации их в тексте нет. После фильтра - потому, что паразит между
        # половинками поправки («Маше, ну, ой, Кате») разрывает параллель, а к
        # этому месту его уже нет.
        #
        # Гейтим уровнем, как и фильтр бубнежа. «Слегка» - это обещание не
        # трогать слова: знаки препинания расставить, ошибку распознавания по
        # словарю поправить, а сказанное оставить как сказано. Удаление
        # оговорки это обещание нарушает - выкидывается кусок собственной речи
        # человека, и решает за него машина. Кто хочет, чтобы за ним
        # подчищали, ставит «средне» - оно и стоит по умолчанию.
