...
# Весь этот модуль - Win32. На macOS `ctypes.WinDLL` не существует вовсе
# (AttributeError на доступе), а `import layered` зовётся из app.py на верхнем
# уровне - поэтому DLL и все argtypes поднимаем только под Windows. Замену этим
# функциям (NSWindow, bundle id, NSWorkspace) когда-нибудь возьмёт macOS-ветка;
# пока пять функций ниже под darwin - честные заглушки, а окно живёт без них.
#
# `wintypes` - под тем же гардом, вопреки прежнему комментарию «есть и на
# macOS». Его там нет: модуль объявляет VARIANT_BOOL с кодом типа "v", а он
# поддержан только на Windows, и `ctypes.wintypes` не импортируется целиком.
# Из-за одной этой строки на маке падали пять модулей, включая app. Поймано
# пробой tools/probe_mac_import.py - глазами по списку это и не находилось.
...
def style_titlebar(hwnd: int, caption=None, dark: bool = True) -> None:
    ...
    if not platform_api.IS_WINDOWS:
        # На macOS заголовок системный, DWM нет: Qt красит окно по теме сам.
        return
    try:
...
def foreground_process() -> str:
    ...
    """
    if not platform_api.IS_WINDOWS:
        # macOS: имя переднего приложения даёт NSWorkspace (bundle id, не exe) -
        # отдельная задача. Пустая строка = правила тона просто не сработают.
        return ""
...
def visible_apps() -> list[tuple[str, str]]:
    ...
    """
    if not platform_api.IS_WINDOWS:
        # macOS: список даёт NSWorkspace.runningApplications - отдельная задача.
        # Пустой список = в настройке «Отправлять сообщение» просто нечего
        # выбрать, окно от этого не ломается.
        return []
    found: dict[str, str] = {}
