// Строка бокового меню - по канону Korti (templates/app-shell): иконка плюс
// текст, gap 11, padding 8/10, радиус 6, кегль 14, вес 500. Активная - заливка
// --fill и чернила, остальные - --text-secondary.
//
// Синим не красим: акцент в системе для особенного (фокус, ссылки), а не для
// «тут я сейчас».

import QtQuick

Item {
    id: nav
    property string label: ""
    property string icon: ""
    property bool active: false
    signal clicked()

    // Ширину диктует панель: пункт растягивается на всю колонку меню.
    width: parent ? parent.width : 212
    height: 34

    activeFocusOnTab: enabled

    // Мышиный ли фокус - разобрано в шапке FocusRing.qml.
    property bool mouseFocus: false
    readonly property bool visualFocus: activeFocus && !mouseFocus
    onActiveFocusChanged: if (!activeFocus) mouseFocus = false

    Keys.onPressed: (e) => {
        if (e.key === Qt.Key_Space || e.key === Qt.Key_Return || e.key === Qt.Key_Enter) {
            nav.clicked();
            e.accepted = true;
        }
    }

    Rectangle {
        id: face
        anchors.fill: parent
        radius: T.radiusSm
        color: nav.active ? T.fill
             : ma.pressed ? T.fill
             : ma.containsMouse ? T.fillSubtle : "transparent"
        Behavior on color { ColorAnimation { duration: T.durFast * 1000 } }

        Row {
            anchors { left: parent.left; leftMargin: 12
                      verticalCenter: parent.verticalCenter }
            spacing: 12

            Icon {
                anchors.verticalCenter: parent.verticalCenter
                path: nav.icon
                size: 17
                color: nav.active ? T.text : T.textMuted
            }
            Text {
                anchors.verticalCenter: parent.verticalCenter
                text: nav.label
                font.family: T.sans
                font.pixelSize: T.tMd
                font.weight: Font.Medium
                color: nav.active ? T.text : T.textSecondary
            }
        }
    }

    FocusRing { target: face; on: nav.visualFocus }

    MouseArea {
        id: ma
        anchors.fill: parent
        hoverEnabled: true
        cursorShape: Qt.PointingHandCursor
        onPressed: { nav.mouseFocus = true; nav.forceActiveFocus() }
        onClicked: nav.clicked()
    }
}
