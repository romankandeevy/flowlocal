...
//
// Тот же компонент стоит в окне заметок, а там контекстного свойства `B` нет
// вовсе (notes_qt заводит только T и L). Одно упоминание `B` в привязке - и
// страница целиком падает в ReferenceError, причём молча: QML напишет про это
// в консоль, которой под pythonw не существует. Поэтому и сочетание, и
// действие приходят снаружи свойствами, а читает их тот, у кого `B` есть.
