...
MultiEffect {
    anchors.fill: parent
    shadowEnabled: true
    // Нейтральная, не тонированная: пилюля висит над чужим экраном, и
    // подкрашенная тень на тёмном фоне светится синим вместо того, чтобы
    // затемнять. Замер и разбор - в шапке theme.py, у SHADOW_OVER.
    shadowColor: T.shadowOver
