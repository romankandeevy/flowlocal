...
Rectangle {
    ...
    Item {
        ...
        Column {
            ...
            // Последняя строка мастера - пожелание, а не указание, и стоит она
            // ниже всех нарочно: человек дочитал до конца настройки, дальше
            // только кнопка «Готово». Прощаться голым «Готово» невежливо.
            //
            // Прячем на время установки: там идёт полоса загрузки на 3.3 ГБ, и
            // желать приятного посреди чужой работы рано. Во всех остальных
            // состояниях - включая «Позже» и неудачу - строка на месте:
            // диктовка работает в любом из них, и мастер для человека кончился.
            Text {
                visible: !llmStep.busy
                width: parent.width
                topPadding: 8
                text: L.t("Приятного использования.")
                font.family: T.sans; font.pixelSize: T.tSm
                color: T.textSecondary
            }
