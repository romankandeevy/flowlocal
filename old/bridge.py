...
class Bridge(QObject):
    ...
    def page_url(self, view: str = "settings", path: str = "/") -> str:
        ...
        `view` говорит, ЧТО показать: settings, wizard, picker, transcript. Их
        четыре, и это четыре разных окна с разной жизнью, а не вкладки одного:
        мастер идёт один раз, picker всплывает поверх чужого окна и живёт
        секунды. Собирать их в один интерфейс с переключателем было бы
        неправдой про то, как ими пользуются.

        Через строку запроса, а не через путь: путь пришлось бы разбирать
        серверу и раздавать один и тот же index.html на четыре адреса, а так
        файл один, а страница смотрит на свой же location.
        """
        return (f"http://127.0.0.1:{self.http_port}{path}"
                f"?t={self.token}&w={self.ws_port}&v={view}")
