...
Это безопасно ровно потому, что порядок такой:
    ...
    1. запомнили окно            <- layered.foreground_window()
    2. сняли текст               <- inserter.grab_text(): выделение, иначе всё поле
    ...
    6. сверили окно и вставили   <- Ctrl+V уходит туда, откуда брали

Пятый шаг не гарантия, а попытка: пока думает модель, человек успевает уйти в
другое окно. Поэтому перед самой вставкой app.py сверяет активное окно с
запомненным и на расхождении не трогает ничего - см. _transform_work.
...
class Picker(QObject):
    ...
    def show(self, items: list, preview: str) -> None:
        """Показать список. Пустой - тоже показ, а не тишина.

        Раньше на пустом списке метод молча выходил, и нажатие сочетания не
        давало вообще ничего - неотличимо от «программа сломалась». Теперь
        пилюля показывает «нету» и говорит, где взять преобразования.
        """
