...
# Указание модели для «привести в порядок»: причесать надиктованную страницу.
# Живёт здесь, в домене заметок; зовёт его settings_qt.Backend.tidyNote через ту
# же правку текста, что и трансформы (app._tidy_note) - второго пути к Ollama не
# заводим. Отдельное от указаний трансформов не случайно: там правки короткие и
# точечные, а здесь целая надиктованная страница, и главная беда у неё другая -
# поток без структуры. Раньше жило в notes_qt.py, вместе с окном; окна больше
# нет (заметки - страница главного окна), а указание перенесено сюда как есть.
TIDY = (
