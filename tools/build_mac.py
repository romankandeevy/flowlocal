...
**Проще запустить из исходников: docs/macos-run.md.** Этот упаковщик ни разу
не отработал до конца, а запуск из исходников показывает ровно то же самое и
без часа возни с подписью. Сюда идти, когда нужен именно .app для раздачи.

    python3 tools/build_mac.py

ЧЕСТНО о состоянии (20.07.2026). Импорт больше не падает: win-only вызовы
убраны под гарды, и `tools/probe_mac_import.py` проходит 20 модулей из 20,
включая `app`. То есть упаковщик, трассирующий импорты, дальше шапки файлов уже
не спотыкается - это и было главной стеной.

Но собранный .app пока не проверен на живом Маке, и запуск - не импорт: на
исполнении ещё ждут `single_instance()` через CreateMutexW, `MessageBoxW`,
фильтр мыши и хоткеи (разбор в docs/macos-port-audit.md, раздел B). Поэтому
скрипт остаётся заготовкой: пробует собрать, честно сообщает, где встал, и
возвращает ненулевой код - job в CI это ожидает (continue-on-error).
...
def main() -> int:
    ...
    # Страница настроек. Без неё окно откроется пустым, и понять почему будет
    # неоткуда: файлов нет, сервер отдаёт честный 404. Та же проверка стоит в
    # flowlocal.spec для Windows.
    if not os.path.exists(os.path.join(ROOT, "web", "dist", "index.html")):
        print("нет web/dist/index.html - сперва соберите страницу:")
        print("    cd web && npm ci && npm run build")
        return 2
