...
def main() -> None:
    ...
    want = render()
    # Пишем с newline="", а ЧИТАЕМ обычным способом, и это не описка.
    #
    # При записи newline="" нужен: без него Windows подставит CRLF, и --check
    # упадёт на файле, который сам же и написал.
    #
    # А вот при чтении он всё ломал. У git на Windows core.autocrlf=true по
    # умолчанию, и свежий `checkout` кладёт файл на диск с CRLF. Здесь на
    # рабочей машине этого не видно - файл лежит таким, каким его написал
    # генератор, то есть с LF, - зато на чистом клоне сравнение падало на
    # переводах строк, ничего не говоря про них. Ровно так и вышло: локально
    # зелено, в CI красно.
    #
    # Обычное чтение приводит любые переводы строк к \n, и сравнивается
    # содержимое, а не то, как его положил на диск git.
    have = None
