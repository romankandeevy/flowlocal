...
week(), calendar(), best_day() и ago() по этим именам - не переименовывать.
...
# Те же месяцы в три буквы - для подписей над столбцами календаря серии. Там на
# месяц приходится 4-5 клеток шириной по десятку пикселей, и «сентября» в них
# не помещается никак. Именительный падеж: подпись стоит сама по себе, а не в
# обороте «12 сентября».
...
def calendar(rows: list[dict], today: date | None = None,
             weeks: int = 26) -> dict:
    """Клетки по дням за последние `weeks` недель: сетка «как на гитхабе».

    Открывается нажатием на серию дней («Статистика»), и отвечает на вопрос,
    на который дуга серии ответить не может: серия говорит «сколько дней
    подряд», а календарь показывает, КАК эти дни прожиты - где густо, где
    пусто и где всё оборвалось.

    Сетка идёт столбцами по неделям, в столбце семь клеток «пн»…«вс», и это
    решает, где она начинается и кончается. Последний столбец - неделя, в
    которой сегодня, поэтому кончается сетка воскресеньем, а не сегодняшним
    днём: иначе последний столбец был бы обрублен посередине и «пт» стояло бы
    в строке «вс». Дни после сегодняшнего помечены future - их рисовать
    нельзя, закрашенная клетка в завтра означала бы, что мы что-то знаем про
    завтра.

    Порядок списка - по датам, и он же порядок обхода сетки сверху вниз,
    столбец за столбцом: внутри недели соседние даты - это соседние дни
    недели. QML раскладывает days подряд в Grid и попадает клетка в клетку,
    без арифметики на JavaScript.

    Насыщенность (level 0..4) - доля от САМОГО плодовитого дня в окне, а не от
    круглого числа вроде сотни слов. Круглое число врало бы обоим: тому, кто
    диктует по три фразы в день, все клетки покрасило бы бледным, а тому, кто
    диктует по часу, - одинаково тёмным. Уровень - это «густо ли по вашим же
    меркам».

    `today` аргументом, а не из системных часов, - как в summary() и week(),
    и по той же причине: на тех же данных ответ обязан быть одинаков что
    сегодня, что через год, иначе это не проверить тестом.
    """
    today = today or date.today()
    # Воскресенье текущей недели: weekday() 0 - понедельник, 6 - воскресенье.
    ...
    while d <= end:
        ...
        days.append({
            ...
            "human": human_date(d),
            "words": words,
            "level": _level(words, peak),
            "today": d == today,
            "future": d > today,
        })
        # Подпись месяца - над столбцом, который этот месяц начинает. Считаем
        # по понедельникам: столбец - это неделя, и принадлежит она тому
        # месяцу, с которого началась. Сверяем номер месяца, а не подпись: на
        # окне длиннее года один и тот же «июл» встретился бы дважды подряд, и
        # сверка по надписи проглотила бы вторую.
        if d.weekday() == 0 and (not months or months[-1]["month"] != d.month):
            months.append({"label": _MONTHS_SHORT[d.month - 1],
                           "col": (d - start).days // 7, "month": d.month})
        d += timedelta(days=1)

    # Первый месяц в окне почти всегда обрезан: полгода назад приходятся на
    # середину месяца, и на него остаётся один-два столбца. Его подпись встаёт
    # вплотную к следующей, и над сеткой читается «янвфев». Убираем ту, что не
    # помещается, - левую: она про две клетки, а правая про целый месяц.
    #
    # Три столбца - это ширина подписи с запасом при самой мелкой клетке.
    # Соседние месяцы отстоят на четыре-пять столбцов, так что правило срабатывает
    # ровно на обрезанном первом, а не режет середину сетки.
    months = [m for i, m in enumerate(months)
              if i + 1 >= len(months) or months[i + 1]["col"] - m["col"] >= 3]

    return {"weeks": weeks, "days": days, "months": months,
            "weekdays": list(_WEEKDAYS), "peak": peak}


def _level(words: int, peak: int) -> int:
    """Насыщенность клетки календаря: 0 - молчали, 4 - как в лучший день.

    Четыре ступени, а не плавная заливка: глаз всё равно не различит сотню
    оттенков зелёного, а различить «густой день» и «обычный» должен с одного
    взгляда. Границы по четвертям от пика - тот же приём, что у гитхаба.
    """
    if words <= 0 or peak <= 0:
        return 0
    share = words / peak
    if share <= 0.25:
        return 1
