...
типографика, кривые движения. `theme_qt.py` пробрасывает их в QML контекстным
свойством `T`, и контролы в `qml/` собраны по спецификациям системы, а не на
глаз.
...
меняет две константы и пересчитывает производные. В QML краски - привязки,
поэтому смена темы перекрашивает окно на лету и ничего не пересобирает.
Настройки -> Основное -> «Оформление»: система / светлая / тёмная.
...
окно. Qt даёт это штатным флагом `WindowDoesNotAcceptFocus`, который на Windows
разворачивается в `WS_EX_NOACTIVATE`. Данные на осциллограмму идут
C++-сигналом внутри процесса - ни IPC, ни сериализации, поэтому 60 кадров в
секунду ничего не стоят.
...
Превью интерфейса - контактный лист со всеми состояниями пилюли в обеих темах:
