...
DEFAULTS: dict = {
    ...
    "llm": {
        ...
        "enabled": None,
        # Чем считать. Два движка, один контракт (cleaner._ask):
        #
        #   ollama  чужая программа с сервером на порту. Работает отлично, но
        #           поставить её человеку, который просто пишет письма, некому;
        #   local   onnxruntime-genai в нашем же процессе, модель в models/
        #           рядом с моделью распознавания. Ни сервера, ни второй
        #           программы - качается по кнопке, как GigaAM.
        #
        # Умолчание - ollama: у тех, у кого она уже стоит, ничего не меняется.
        "backend": "ollama",
        "url": "http://127.0.0.1:11434",
        # Пусто - выберем из установленных сами (cleaner.pick_llm_model).
        "model": "",
        # Папка своей модели в models/ - для backend = local.
        "local_model": "",
        "timeout_sec": 20,
        # Потолок ответа для своей модели. У Ollama его держит сам сервер.
        "max_new": 512,
    },
}
