...
datas = [
    ...
    ("CHANGELOG.md", "."),
    # Архив 0.x под тем же docs/, что и в исходниках: «О программе» показывает
    # его выплывающей панелью «Прежние версии». Папку сохраняем (docs -> docs),
    # чтобы путь чтения (base/docs/history-0.x.md в settings_qt.oldVersions) был
    # один и в сборке, и из исходников. Нет файла - панель просто не появится.
    ("docs/history-0.x.md", "docs"),
    # QML: PyInstaller ищет импорты в .py и про .qml не знает.
    #
    # Папка кладётся ЦЕЛИКОМ и вместе с вложенностью - это важно с 19.07.2026,
    # когда qml/ поделилась на controls/ и windows/. Окна тянут контролы
...
hiddenimports = [
    ...
    # Расшифровка файла (PLAN 11). Обе библиотеки - cffi, то есть питоновский
    # модуль плюс скомпилированное расширение рядом, и импортируются они не с
    # первой строки app.py, а лениво, из audiofile. Ленивый импорт статический
    # анализ и так видит, но расширения у cffi зовутся не как пакет
    # (`_miniaudio`, `_soundfile_data`), и терять их в сборке - обычное дело.
    #
    # Цена ошибки здесь несимметричная: лишняя строка стоит ноль, а пропажа
    # вылезает только у человека с установщиком и только когда он перетащит
    # первый файл, - то есть там, где чинить уже поздно.
    "_miniaudio",
    "miniaudio",
    "_soundfile_data",
    "soundfile",
